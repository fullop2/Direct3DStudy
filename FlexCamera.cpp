...
FlexCamera::FlexCamera(IDirect3DDevice9* device)
	:Camera(device),mCameraType(AIRCRAFT),mLook(D3DXVECTOR3{ 0,-100,0 }),mRight(D3DXVECTOR3{1.0f,0.0f,0.0f})
{
	mUp = D3DXVECTOR3{ 0.0f,1.0f,0.0f };
	mPosition = D3DXVECTOR3(0, 100, 0);
}
...
{
	static float speed = 1.0f * Timer::Get().GetTime() / 1000.0f;
	if (GetKeyDown(16))
	{
		speed = speed *0.3f;
	}
